...
public class OptimizedSearchAlgorithm {
	 ...
	 *  not the most optimized solution. The algorithm will only generate one solution at a time (even if
	 *  in some cases different path has same weight) */
	public ArrayList<Coordinate> OptimizedSearchAlgorithm(SmartPiece p) {
...
//					System.out.println(" Turning point | can move to next :"+tempCoor.x+" "+tempCoor.y);
					...
					} else { // no suggestion TODO: move down temporarily
						tempCoor.y--;
						optimzedPath.add((needTransform) ? new Coordinate(tempCoor.y, tempCoor.x) : new Coordinate(tempCoor.x, tempCoor.y));
						break;
					}
